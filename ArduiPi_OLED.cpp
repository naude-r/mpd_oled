...
void ArduiPi_OLED::begin( void )
{
  ...
  sendCommand(SSD1306_Set_Higher_Column_Start_Address, 0xB0, 0xC8);     /*set higher column address*/
  sendCommand(SSD1306_Set_Lower_Column_Start_Address, 0x10, 0x40); /*set lower column address*/
  sendCommand(SSD_Set_Segment_Remap); /*set segment remap*/
  sendCommand(SSD1306_Normal_Display);     /*normal / reverse*/
  sendCommand(SSD_Set_Muliplex_Ratio, 0x3F);
  ...
  sendCommand(SSD1306_Set_Display_Clock_Div, 0xF0);
  sendCommand(SSD1306_Set_Precharge_Period, 0x22);
  sendCommand(SSD1306_Set_Com_Pins, 0x12);    /*set COM pins*/
  sendCommand(SSD1306_Set_Vcomh_Deselect_Level, 0x20);    /*set vcomh*/
  sendCommand(SSD1306_Charge_Pump_Setting, 0x14);
  //sendCommand(SSD1306_Set_Com_Output_Scan_Direction_Remap);
  sendCommand(SSD_Set_ContrastLevel, 0x7F);
...
void ArduiPi_OLED::sendData(char* c, uint32_t len) {
  if ( isSPI())
  {
    // SPI
    // Setup D/C line to high to switch to data mode
...
void ArduiPi_OLED::display(void)
{
  ...
  {
    sendCommand(SH1106_Set_Page_Address+k);//set page addressSSD_Data_Mode;
